...
<html>
<head>
    <title>Edit Reading Exam - Admin</title>
    ...
    <style>
        ...
        @media (max-width: 900px) {
            .container-flex { flex-direction: column !important; }
...
<body>
    <div class="admin-header">
        <div class="container d-flex align-items-center justify-content-between">
            ...
            <h1 class="mb-0">Edit Reading Exam</h1>
        </div>
    </div>
    <div class="container">
        <% 
        ...
        <form action="${pageContext.request.contextPath}/admin/exam-management" method="post" enctype="multipart/form-data" class="edit-form-container">
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <select name="type" id="examType" class="form-select mb-3" required>
                        <option value="READING_FULL" <%= "READING_FULL".equals(exam.getType()) ? "selected" : "" %>>Reading Full</option>
                        <option value="READING_SINGLE" <%= "READING_SINGLE".equals(exam.getType()) ? "selected" : "" %>>Reading Single</option>
                    </select>
                </div>
                <div class="col-md-6">
                    <div class="card">
            ...
            <% if ("READING_FULL".equals(exam.getType()) && passages.size() > 1) { %>
            ...
            <div class="section-container" id="section-<%= passage.getSection() %>" style="<%= passage.getSection() == 1 ? "" : "display: none;" %>">
                <div class="container-flex d-flex gap-3 px-3">
                    <div class="left-panel w-50">
                        <h4>Section <%= passage.getSection() %> - Passage Editor</h4>
                        <input type="hidden" name="passageId_<%= passage.getSection() %>" value="<%= passage.getPassageId() %>">
                        <label class="form-label">Title:</label>
                        <input type="text" name="passageTitle_<%= passage.getSection() %>" value="<%= passage.getTitle() != null ? passage.getTitle() : "" %>" class="form-control" required>
                        <label class="form-label mt-2">Content:</label>
                        <textarea name="passageContent_<%= passage.getSection() %>" id="passageContent_<%= passage.getSection() %>" rows="15" class="form-control" required><%= passage.getContent() != null ? passage.getContent() : "" %></textarea>
                    </div>
                    <div class="right-panel w-50">
                        <h4>Section <%= passage.getSection() %> - Question Groups</h4>
