...
function addQuestionGroup() {
  while (activeGroups.has(groupIndex)) groupIndex++;
...
function addQuestion(groupId) {
  ...
  if (type === "MULTIPLE_CHOICE") {
    ...
    ["SHORT_ANSWER", "SENTENCE_COMPLETION", "MATCHING_ENDINGS"].includes(type)
  ) {
    html += `
  ...
  } else if (["FLOWCHART", "TABLE_COMPLETION", "DIAGRAM_LABELING", "MATCHING_HEADINGS"].includes(type)) {
    html += `
  ...
  if (type === "MULTIPLE_CHOICE") {
    for (let i = 0; i < 4; i++) addOption(groupId, questionId);
  }
}
